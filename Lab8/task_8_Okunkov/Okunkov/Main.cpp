...
// создаем буферный объект, идентификатор которого присваиваем vertexBuffer
	...
	while (!glfwWindowShouldClose(window)) { // пока окно window не должно закрыться
		...
		shaderProgram.setUniform("light.diffuse", glm::vec3(0.5f * k, 0.5f * k, 0.5f * k));
		shaderProgram.setUniform("light.specular", glm::vec3(1.f * k, 1.f * k, 1.f * k));
		...
		// белый цвет
		myLightShaderProgram.setUniform("pathColor", glm::vec3((0.2f, 0.2f, 0.2f) + (0.5f * k, 0.5f * k, 0.5f * k) + (1.f * k, 1.f * k, 1.f * k)));
		glBindVertexArray(lightVertexArray); // делаем активным вершинный массив
		...
		// белый цвет
		lightShaderProgram.setUniform("pathColor", glm::vec3((0.2f, 0.2f, 0.2f) + (0.5f * k, 0.5f * k, 0.5f * k) + (1.f * k, 1.f * k, 1.f * k)));
		glBindVertexArray(lightVertexArray); // делаем активным вершинный массив
		glDrawArrays(GL_LINES, 0, 26); // рисуем 13 отрезков (в массиве 26 точек)
		glBindVertexArray(0); // отключаем вершинный массив
