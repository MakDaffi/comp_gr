...
namespace Okunkov {
	...
	// начальная отрисовка в матричной форме
	vector<model> models;
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->btnOpen->Location = System::Drawing::Point(253, 12);
...
#pragma endregion
	...
	private: System::Void MyForm_KeyDown(System::Object^ sender, System::Windows::Forms::KeyEventArgs^ e) {
		// вычисление координат центра текущего окна
		float Wcx = Wx / 2.f + left; // координаты центра
		float Wcy = Wy / 2.f + top; // текущего окна

		switch (e->KeyCode) { //кнопка для смены рисунка
		case Keys::Q:
		...
		case Keys::T:
			T = translate(0.f, -10.f) * T; // сдвиг вверх на десять пикселей
			break;

		case Keys::F:
			T = translate(-10.f, 0.f) * T; // сдвиг влево на десять пикселей
			break;

		case Keys::G:
			T = translate(0.f, 10.f) * T; // сдвиг вниз на десять пикселей
			break;

		case Keys::H:
			T = translate(10.f, 0.f) * T; // сдвиг вправо на десять пикселей
			break;

		case Keys::Z:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1.1f) * T; // увеличение изображения относительно текущего центра окна в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::X:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1 / 1.1f) * T; // уменьшение изображения относительно текущего центра окна в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::U:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = mirrorX() * T; // зеркальное отражение изображения относительно горизонтальной оси, проходящей через центр окна
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::J:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = mirrorY() * T; // зеркальное отражение изображения относительно вертикальной оси, проходящей через центр окна
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::I:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1.1f, 1.f) * T; // растяжение изображения по горизонтали относительно текущего центра окна в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::K:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1 / 1.1f, 1.f) * T; // сжатие изображения по горизонтали относительно текущего центра окна в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::O:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1.f, 1.1f) * T; // растяжение изображения по вертикали относительно текущего центра окна в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::L:
			T = translate(-Wcx, -Wcy) * T; // перенос начала координат в (Wcx, Wcy)
			T = scale(1.f, 1 / 1.1f) * T; // сжатие изображения по вертикали относительно текущего центра окна в 1.1 раза
			T = translate(Wcx, Wcy) * T; // перенос начала координат обратно
			break;

		case Keys::Escape: // сброс всех сделанных преобразований
	...
	private: float left = 30, right = 100, top = 20, bottom = 50; // расстояния до границ окна
		   ...
		   float Wx, Wy; // ширина и высота прямоу
