...
public:
	...
	vec3& operator*=(const vec3& v) { //умножение векторов *= и *
		x *= v.x;
		y *= v.y;
		z *= v.z;
		return *this;
...
public:
	...
	} //благодар€ перегрузке оператора [] мы можем работать с каждым элементом матрицы

	//определение операции транспонировани€
	mat3 transpose() {
	...
	// определение операций произведени€ матриц *= и =
	...
	const mat3 operator* (const mat3& m) { //перегружаем оператор * благодар€ предыдущей функции
		return mat3(*this) *= m;
	}
};
